...
ingest:
	...
	-window_size 5  -extra_window_size 10 -remove_at_exit 1 -adaptation_sets "id=0,streams=v id=1,streams=a" -f flv rtmp://localhost:1935/stream/foo?psk=bar

run-live:
	docker-compose up

.PHONY: create-db ingest run-live